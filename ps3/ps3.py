...
def get_word_score(word, n):
    ...
    first_component = 0                                                         # Calculate value of first component 
    for i in word_lower: 
        try:
            first_component += SCRABBLE_LETTER_VALUES[i]
        except: 
            print('Dictionary find has problems.')
        
    second_component = 7*len(word)-3*(n-len(word))                              # Calculate value of second component
    if second_component < 1: 
        second_component = 1
        
    score = first_component*second_component                                    # Calculate and return score 
    return score 
...
def update_hand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    word = word.lower()                                                         # Convert word to all lower case
    updated_hand = hand.copy()                                                  # Create a copy of hand
     
    for i in word:                                                              # Iterate through word and remove values from hand
        if (updated_hand.get(i,0) != 0):                                        # If exists in dictionary decrement letter and remove from dictionary when no letters
            updated_hand[i] -= 1
...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()                                                         # Convert word to all lower case
    possible_words = []                                                         # Array of all possible combination of words
    wordlist_flag = False
    possible_wordlist_words = []
    wild_flag = 0
    invalid_count = 0                                                           
    
    if '*' in word:                                                             # Case of word having an '*'
        wild_flag = 1
        for i in range(len(VOWELS)):
            possible_words.append(word.replace('*',VOWELS[i]))
    ...
    for i in possible_words:                                                    # Check if word is in wordlist
        if i in word_list:
    ...
    if invalid_count == 5 and wild_flag == 1:        
        return False                                                           
    elif wild_flag == 0 and invalid_count == 1:
        return False
    else:
        return True
#
    ...
    while calculate_handlen(hand) > 0:
        ...
        if word == '!!':
            ...
            if is_valid_word(word, hand, word_list):                            # If the word is valid:
                round_points = get_word_score(word, calculate_handlen(hand))    # Calculate Score
                score += round_points
                try:
                    print('"'+ word +'"','earned', round_points,'points. Total:',
                      score, 'points')                                          # Tell the user how many points the word earned,
                except: 
                    print('report error')
